namespace Morozov.WebApiSample
...
type Startup private () =
    ...
    member this.ConfigureServices(services: IServiceCollection) =
        ...
        let controllerOptions =
            System.Action<MvcOptions>(fun options ->
                options.OutputFormatters.Clear()
                options.InputFormatters.Add(XmlSerializerInputFormatter(options))
                options.OutputFormatters.Add(XmlDataContractSerializerOutputFormatter()))
        services.AddControllers(controllerOptions) |> ignore
